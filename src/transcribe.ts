...
async function transcribeChunked(
  ...
  try {
    ...
    await execAsync(
      `ffmpeg -i "${filePath}" -f segment -segment_time 600 -c:a libmp3lame -b:a 128k "${chunkPattern}" 2>/dev/null`
    );
...
export async function extractAudio(videoPath: string): Promise<string> {
  ...
  try {
    await execAsync(
      `ffmpeg -i "${videoPath}" -vn -acodec libmp3lame -b:a 128k "${audioPath}" 2>/dev/null`
    );
    return audioPath;
  } catch (error) {
 ...
 */
export async function extractAudioFromUrl(url: string): Promise<string> {
  ...
  try {
    await execAsync(
      `/usr/local/bin/yt-dlp ${cookieArg} -x --audio-format mp3 --audio-quality 128K -o "${audioPath}" "${url}" 2>/dev/null`,
    ...
    if (existsSync(audioPath + '.mp3')) return audioPath + '.mp3';
    
    throw new Error('Audio file not created');
  } catch (error) {
    throw new Error(`Failed to extract audio from URL: ${(error as Error).message}`);
  }
}
