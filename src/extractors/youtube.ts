...
import type { Extractor, ExtractedContent, Timestamp } from '../types.js';
...
export const youtubeExtractor: Extractor = {
  ...
  async extract(url: string): Promise<ExtractedContent> {
    ...
    try {
      ...
      throw new Error(`Failed to fetch YouTube metadata: ${(error as Error).message}`);
    }
    ...
    if (!transcript && process.env.OPENAI_API_KEY) {
      try {
        ...
        console.log('Whisper transcription failed:', (whisperError as Error).message);
 ...
 */
function extractTimestamps(text: string): Timestamp[] {
