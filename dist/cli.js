...
import { generateFilename, formatHtml, formatMarkdown } from './format.js';
const VERSION = '0.1.0';
program
    .name('shrd')
    ...
    const { output, format: outputFormat, style, tone, perspective, extractOnly, verbose, info: infoOnly, } = options;
    try {
        ...
        if (extractOnly) {
            ...
            result = {
                ...
                markdown: formatMarkdown({ title: extracted.title, description: extracted.description || '', insights: [], quotes: [] }, normalized),
            };
        }
        else {
            // Full processing
        ...
        if (output) {
            ...
            const filepath = output.endsWith('/')
                ? join(output, generateFilename(result.blog.title, outputFormat))
                : output;
