 ...
 */
async function extractDirectAudio(url) {
 ...
 */
async function extractGenericPodcast(url) {
    ...
    try {
        ...
        let metadata;
        try {
            const { stdout } = await execAsync(`/usr/local/bin/yt-dlp ${cookieArg} --skip-download -j "${url}" 2>/dev/null`, { env: { ...process.env, PATH: `${process.env.HOME}/.deno/bin:${process.env.PATH}` } });
        ...
        await execAsync(`/usr/local/bin/yt-dlp ${cookieArg} -x --audio-format mp3 --audio-quality 128K -o "${tempAudio}.%(ext)s" "${url}" 2>/dev/null`, { env: { ...process.env, PATH: `${process.env.HOME}/.deno/bin:${process.env.PATH}` } });
