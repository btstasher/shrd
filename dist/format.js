 ...
 */
export function format(blog, content, outputFormat = 'markdown') {
 ...
 */
export function formatMarkdown(blog, content) {
    const lines = [];
    // Title
    ...
    if (blog.mentions && blog.mentions.length > 0) {
        ...
        lines.push('');
        lines.push(blog.mentions.map(m => `\`${m}\``).join(' • '));
        lines.push('');
    }
    // Embed section
 ...
 * Format as HTML
 */
export function formatHtml(blog, content) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  <article>
    ...
    <h2>Key Insights</h2>
    <ul>
      ${blog.insights.map(i => `<li>🎯 ${escapeHtml(i)}</li>`).join('\n      ')}
    </ul>
    ` : ''}
 ...
 * Format as JSON
 */
export function formatJson(blog, content) {
 ...
 */
function getWatchReadListenLabel(platform) {
    switch (platform) {
        case 'youtube':
        case 'tiktok':
        case 'instagram':
 ...
 */
export function generateFilename(title, format = 'markdown') {
    const slug = title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
